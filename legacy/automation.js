...
const { initializeSlackCommands } = require('../src/slackCommands');
const { registerTasksCommand } = require('../src/tasksCommand');
const { handleWebhook } = require('../src/webhookHandler');
const { formatSlackMessage } = require('../src/messageFormatter');
...
async function startServer() {
  try {
    ...
    logger.success(`⚡️ LEGACY SERVER running on port ${PORT}`);
    logger.success('✅ Commands: /create-task, /quick-task, /monday-help, /tasks');
    logger.warn('⚠️ WARNING: This is the legacy automation app - consider using src/server.js instead');
  } catch (error) {
...
receiver.app.get('/', (req, res) => res.json({ message: 'LEGACY Monday → Slack Automation', version: '4.7.0-legacy', status: 'running', lastRun: metrics.lastRun }));
