...
const logger = {
  ...
  warn: (msg, data = {}) => {
    console.log(JSON.stringify({ level: 'warn', message: msg, data, timestamp: new Date().toISOString() }));
  },
  error: (msg, error = {}) => {
    console.error(JSON.stringify({ level: 'error', message: msg, error: error.message || error, stack: error.stack, timestamp: new Date().toISOString() }));
  },
  success: (msg, data = {}) => {
    console.log(JSON.stringify({ level: 'success', message: msg, data, timestamp: new Date().toISOString() }));
  }
};
...
function formatSlackMessage(tasks, userName) {
  ...
  if (tasks.overdue.length > 0) {
    ...
    tasks.overdue.slice(0, 5).forEach(task => {
      ...
      blocks.push({ type: 'section', text: { type: 'mrkdwn', text: `*${task.name}*\n📅 ${dueDate} | 📍 ${task.boardName}` } });
    });
  }
  ...
  blocks.push({ type: 'divider' });
  blocks.push({ type: 'actions', elements: [{ type: 'button', text: { type: 'plain_text', text: '📱 Open Monday.com', emoji: true }, url: 'https://drexcorp-company.monday.com', style: 'primary' }] });
...
async function sendOrUpdateSlackMessage(user, slackMessage) {
  try {
    ...
    if (storedMessage && storedMessage.channelId === channelId) {
      await slack.chat.update({ channel: channelId, ts: storedMessage.messageTs, blocks: slackMessage.blocks, text: `${user.name}'s Tasks for Today` });
      metrics.messagesUpdated++;
      logger.success(`Updated message for ${user.name}`, { channelId });
    } else {
      const response = await slack.chat.postMessage({ channel: channelId, blocks: slackMessage.blocks, text: `${user.name}'s Tasks for Today` });
    ...
    if (error.data?.error === 'users_not_found' || error.data?.error === 'account_inactive' || error.data?.error === 'invalid_email' || error.data?.error === 'user_disabled') {
      logger.warn(`Slack issue for ${user.name}: ${error.data.error}`, { userId: user.id, email: user.email });
...
receiver.app.post('/trigger', async (req, res) => {
  ...
  res.json({ status: 'triggered', message: 'Automation started', timestamp: new Date().toISOString() });
  runAutomation().catch(error => logger.error('Manual trigger failed', error));
});

receiver.app.get('/health', (req, res) => {
  res.json({ status: 'ok', timestamp: new Date().toISOString(), uptime: process.uptime(), lastRun: metrics.lastRun, metrics: { usersProcessed: metrics.usersProcessed, messagesUpdated: metrics.messagesUpdated, messagesSent: metrics.messagesSent, webhooksReceived: metrics.webhooksReceived, notificationsSent: metrics.notificationsSent, errors: metrics.errors } });
});

receiver.app.get('/', (req, res) => {
  res.json({ message: 'Monday.com → Slack Automation Service', status: 'running', version: '4.6.0', mode: 'scheduled', schedule: '9:00 AM EST weekdays', endpoints: { health: '/health', trigger: '/trigger (POST)', triggerPage: '/trigger.html (GET)', webhook: '/webhook/monday (POST)', slack: '/slack/events (Slack commands)' }, lastRun: metrics.lastRun });
});

receiver.app.get('/metrics', (req, res) => {
  res.json({ ...metrics, uptime: process.uptime(), timestamp: new Date().toISOString() });
});
